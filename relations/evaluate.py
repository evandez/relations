...
def evaluate(
    ...
    if(validation_set == None):
        ...
        print(f"{relation_id} >> number of requests in counterfact = {len(cf_relation)}")
        
        print("Checking correct prediction with normal calculation ...")
        correct_predict = filter_correct_predictions_by_model(
            model = model, tokenizer = tokenizer,
            relation_prompt = relation_prompt, cf_relation = cf_relation
        )
        print(f"Number of correctly predicted requests = {len(correct_predict)}")
        validation_set = [(c['subject'], -1, c['target_true']['str']) for c in correct_predict]
